...
export interface IProps {
  ...
  transcode: (name: string, type?: string, fileName?: string) => void;
  /**
...
export const UiFileInputButton: React.FC<IProps> = (props) => {
  ...
  const onChangeHandler = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Array.from(event.target.files).forEach((file) => {
      ...
      console.log('form', event.target.name, file);
    });
    ...
    await props.transcode(name, event.target.files[0].type, event.target.name);
